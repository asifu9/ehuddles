...
/**
 * <p>A java version of thumbnail generator http://www.grails.org/plugin/image-tools.</p>* 
 * @author sverdianto
 * @see http://www.grails.org/plugin/image-tools
 * @version 1.0.4
 *
 */
public class ImageToolThumb {
    ...
    public void loadMask(String file) throws IOException {
        mask = (RenderedOp) ImageToolThumb.masks.get(file);
        if (mask == null) {
            FileSeekableStream fss = new FileSeekableStream(file);
    ...
    public void loadAlpha(String file) throws IOException {
        alpha = (RenderedOp) ImageToolThumb.alphas.get(file);
        if (alpha == null) {
            FileSeekableStream fss = new FileSeekableStream(file);
     ...
     * @see <a *="" href="http://java.sun.com/products/java-media/jai/iio.html">Possible
     *      JAI encodings</a>
     */
    ...
    public void thumbnail(float edgeLength) {
        if (getHeight() < edgeLength && getWidth() < edgeLength && decreaseOnly) {
            ...
            float modifier = edgeLength
                    / (float) (tall ? getHeight() : getWidth());
     ...
     */
    public void thumbnailSpecial(float maxWidth, float maxHeight,
            int interPolationType, int renderingType) {
        if (getHeight() <= maxHeight && getWidth() <= maxWidth) {
            ...
            float modifier = maxWidth
                    / (float) (tall ? (getHeight() * (maxWidth / maxHeight))
                            : getWidth());
            ParameterBlock params = new ParameterBlock();
            params.addSource(image);
