 ...
 */
@javax.servlet.annotation.WebServlet(urlPatterns = { "/UserFeedInfo" }, asyncSupported = true)
public class UserFeedInfo extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, final HttpServletResponse resp) throws ServletException, IOException {
    	...
    	//System.out.println("UserFeedInfo in Asyn servlet ");
        ...
        EmpInfo emp=null;
        emp=Session_control.getSession(req);
...
"}"+
        ...
        response.getWriter().write(head);
        response.getWriter().flush();
        response.getWriter().write("<script type=\"text/javascript\">function arrived(id,type,text) {"+
        		//"{alert('hi');"+
        		...
        		"else if(type='basic'){"+
        		"addDynamicBasicInfo(newJ);}"+
        		"else if(type=='test'){"+
        		"addPhotoShowForUser(text);}"+
        		"else if(type='birth'){"+
        		"addMyDynamicBirthEvents(newJ);}"+
        		" }</script>");
        response.getWriter().write(head);
        response.getWriter().flush();
        response.getWriter().write("<script type=\"text/javascript\">function registerMyEvents(){registerEvents();}</script>");
       // writer.write("<script type=\"text/javascript\">function arrivedTicker(id,text) {alert('hi');var newT= $.parseJSON(text); addMyTickerDynamically("+albumPath+",newT);var b=document.getElementById(id); }</script>");
        response.getWriter().write("<script type=\"text/javascript\">function arrivedLeftHeaderContent(data){alert(\"hi\");var b=document.getElementById(\"insertPost\");}</script>");
        response.getWriter().write("</HEAD><BODY  marginheight=\"0\" marginwidth=\"0\">");
        response.getWriter().flush();
        String body="";
	  ...
	  "</div>";
        ...
        response.getWriter().flush();
       body=""+
       "<div style=\"display: table; width: 100%;vertical-align: top;\" align=\"center\" class=\"newBackgroundColor\">"+
...
//"<div id=\"userInfo\" class=\"tickerPopup\"  ></div>"+
 ...
 "";
        ...
        LeftHeaderMethodaa ob= new LeftHeaderMethodaa(ctx.getResponse().getWriter(), emp,resp);
        RightBirthDays ob3=new RightBirthDays(ctx.getResponse().getWriter(),resp);
        //TickerRightSection ob2=new TickerRightSection(writer);
        UserBasicInfo ob4=new UserBasicInfo(ctx.getResponse().getWriter(), emp,resp);
       ...
       tasks.add(ob3);
        ...
        response.getWriter().write(" <div id=\"dynaPopup\" ></div> ");
        response.getWriter().write(" <div id=\"popupContact\" style=\"width: 50%;height: 50%;\" align=\"center\">");
        response.getWriter().write("<a id=\"popupContactClose\" class=\"redbuttonColor\">x</a>");

        response.getWriter().write("<iframe style=\"margin: 0px;\" width=\"100%\" scrolling=\"no\" height=\"100%\" id=\"myIFrame\" frameborder=\"0\" ></iframe>");
        response.getWriter().write("</div>");
        response.getWriter().write("<div id=\"backgroundPopup\" style=\" height: 100%;\"></div> ");
        ...
        response.getWriter().write(lastSection);
        response.getWriter().flush();
        ctx.complete();
    }
...
//        	writer.flush();}
    ...
    /**------------------------------------------------------------------------------*/
    // for main pagelet
     class LeftHeaderMethodaa implements Callable<Boolean>{
    	
    	 EmpInfo info;
    	 PrintWriter writer;
    	HttpServletResponse resp;
    	public LeftHeaderMethodaa(PrintWriter w,EmpInfo info,HttpServletResponse resp) {
			// TODO Auto-generated constructor stub
    		this.info=info;
    		this.writer=w;
    		this.resp=resp;
    	...
    	public Boolean call() throws Exception {
    		...
    		pageletLeftHeaderContentsss();
    		return false;
    	}
    	
    	private void pageletLeftHeaderContentsss() {
    		...
    		try {
    			...
    			List<PostInfo> p  = new ArrayList<PostInfo>();
    			int count =0;
	...
	//        			}
	    			...
	    			System.out.println(" value si " + resp);
    ...
    // for main pagelet
     class UserBasicInfo implements Callable<Boolean>{
    	 ...
    	 PrintWriter writer;
    	HttpServletResponse resp;
    	public UserBasicInfo(PrintWriter w,EmpInfo info,HttpServletResponse resp) {
			// TODO Auto-generated constructor stub
    		this.info=info;
    		this.writer=w;
    		this.resp=resp;
...
//        		}
    			...
    			String dd =info.getId();
    			//TODO need to fix this
    			//dd=dd.replaceAll("\\","/");
    			//System.out.println(" here i am " +dd);
    		//	infos.setCoverPage(PhotoInfoDAO.getPhotoByIdDynamic(dd, con));
