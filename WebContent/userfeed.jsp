 ...
 EmpInfo empOther=null;
	HashMap<String,Boolean> roleMap=new HashMap<String,Boolean>();
		if(Session_control.getSession(request)!=null){
			...
			empOther =Session_control.getOtherInfo(request);
...
<script type="text/javascript">
    var empId='<%= empOther.getId() %>';
    var imagePPPPath = '<%=empOther.getImagePath()%>';
    var paaaaath = '<%=PropertyReader.getValue("photoAlbumPath",request)%>';
   
   
    $.getJSON("GetPost", {'key' :empId,'flow':'0'}, function(datas) { // Do an AJAX call
    	

    	
    	$.each(datas, function(k,item){
    		addMyPostDynamicallyInUserFeed(item);
			
			
 			 });
    
	  });
    $.getJSON("GetUserBasicInfo",{'id':empId},function(data){
    	var info= data.info;
    	//alert("data.followers " + data.followers);
    	var follow="";
     
    //	var birth=data.emp;
   // 	var photo=data.photo;
     	addDynamicBasicInfo(info,follow);
		...
		<div class="container" style="margin: 0;">
			...
			<div class="collapse navbar-collapse navHeaderCollapse">
				...
				<ul class="nav navbar-nav navbar-right">
					<li class="active"><a href="feeds.jsp"><i
							class="fa fa-home fa-2x"></i>Home</a></li>
					<li><a href="profilefeed.jsp"><i class="fa fa-user fa-2x"></i>
							Profile</a></li>
					<li onclick="showLinks()"><a id="links"
						onclick="$('#manageLinks').modalPopover('show')"><i
							class="fa fa-home fa-2x"></i>link</a></li>
					...
					<div class="panel-body"
						...
						<div style="vertical-align: top; width: 100%; padding-left: 5px;"
							...
							<input type="hidden" id="txtPhotoPath"
								...
								type="hidden" id="txtEmpIdTarget" value="<%= empOther.getId()%>">
