...
function PushClient(ch, m){
	...
	PushClient.prototype.login = function(){
		...
		try{
			...
			this.ajax.setRequestHeader("keep-alive", "timeout=999, max=999");
	...
	function handleMessage() {
		...
		try{
			if(this.readyState == 0){
				...
				if (this.status == 200){
					this.pushClient.onMessage("" + this.responseText.substring(this.responseText.lastIndexOf("|")));
				}else{
					//this.pushClient.onMessage("" + this.status + "/-");
				}
