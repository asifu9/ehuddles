...
$(document).ready(function () {
	 ...
	 $('#insertGroups').slimScroll({
         height: '500px',
         alwaysVisible: false,
         size: '8px',
         color: '#555555'
         });
	 $('#groupMembers').slimScroll({
         height: '250px',
         alwaysVisible: false,
         size: '8px',
         color: '#555555'
...
function addUserMemberDynamically(item){
	
	var base = document.getElementById("basePathOfPhoto").value;
	var str='';
			  		if(item.key!=null){
			  		 var p='images/person_icon.jpg';
			  			if(item.userIdInfo.imagePath!=null)
			  				p=base+item.userIdInfo.imagePath.photoPath.replace("0_","7_");
			  				str+='<div id="'+item.key+'"  class="birthdaySection"  ><div id="Abc" style="display:table-row;" ><div style="display:table-cell;padding-left:3px;vertical-align: top;">';
				  			str+='<img src="'+p+'" width="30px" height="30px" ></div><div style="display:table-cell;vertical-align: top;padding-left:5px;"><a href="UserOtherFeedInfo?u='+item.userId+' "><span class="birthdayNameStyle" style="vertical-align: top;">';
				  			str+=item.userIdInfo.empName+' ' + '</span></a> <br> joined on: &nbsp;<span class="tickerMessageStyle" style="vertical-align: top;">'+item.joinedOnStr+' </span></div>';
				  			str+='</div></div>';
			  			
			  		}
				 $("#insertMembers").append(str);
	    ...
	    if(event.keyCode == 13){
	    	...
	    	if(commentDesc=="" ||commentDesc==null)
	    	 ...
	    	 $.getJSON("CreateCommentForGroupPost", {'commentedById' :id,'commentDesc':commentDesc,'postId':key,'flow':1}, function(datas) { // Do an AJAX call
	    		  	var photo ='';
	    		  	if(datas.commentUserPhoto==null || datas.commentUserPhoto=='null')
	    		  		photo='images/person.jpg';
	    		  	else
	    		  		photo=basePath + datas.commentUserPhoto;
...
function hideDeleteButton(key){
 ...
 function addMyGroupPostDynamically (item){
	...
	if(item.likes!=null){
		...
		var image ='images/person.jpg';
		if(item.postedByIdInfo.imagePath!=null && item.postedByIdInfo.imagePath!='null')
			image =paaaaath+item.postedByIdInfo.imagePath.photoPath.replace("0_","7_");
		
		str+='<div class="imageSection"> <img src="'+image+'" width="50px" height="50px"></div> ';
...
//	}else
	...
	if(item.comments!=null){
	 ...
	 $.each(item.comments,function(j,comm){
		...
		str+='<div class="photoSpaceStyle"><img src="'+paaaaath +commentUserPhoto+'" width="50px" height="50px">';
		str+='</div></div>';
	...
	str+='<div class="commentPhotoStyle" width="10px"><div class="photoSpaceStyle"><img src="'+ image+'" width="50px" height="50px"></div></div>';
...
function submitGroupPost(){
	...
	var qStringIs= 'postDesc='+data +
					'&userId='+id+''; 
	var bPast= document.getElementById("basePathOfPhoto").value;
	imagePath=bPast+imagePath.replace("0_","7_");
...
function addGroupDivDynamically(item){
	...
	if(item.photoId!=null)
		p=protocol+'//'+window.location.host+'/PhotoStatic/userUploads/'+item.photoIdInfo.photoPath.replace("0_","6_");
...
function addOthersGroupDivDynamically(item){
	...
	//alert(" item.photoid " + item.photoId);
	if(item.photoId!=null && item.photoId!='null'){
		//alert( "item.photoIdInfo.photoPath " + item.photoIdInfo.photoPath);
		p=protocol+'://'+window.location.host+'/PhotoStatic/userUploads/'+item.photoIdInfo.photoPath.replace("0_","6_");
...
//		    	  });
 ...
 function shareMyPhoto(key){
	 ...
	 var path=protocol+'://'+window.location.host+'/PhotoStatic/userUploads/';
