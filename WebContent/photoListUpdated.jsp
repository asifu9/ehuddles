...
function hidedeleteButton(key){
  ...
  function getDescription(id){
	  ...
	  commentsForPhoto(id);
	 var isFound=0;
  $.getJSON("GetPhotoDescription", {inputQueryString : queryString}, function(datas) { // Do an AJAX call
 ...
 <script type="text/javascript">
    ...
    for(PhotoInfo p:list){
    	...
    	if(tt=='<%=path%>')
    		currentId=i+1;
    	i=i+1;
    	plist[i]= '<%= PropertyReader.getValue("photoAlbumPath",request)%>'+'<%=p.getPhotoPath()%>'+'~~~'+'<%=p.getIdPhotoInfo()%>';
    ...
    %>
      function switchImage(i,option)
      {
		...
		if(option==-1){
			...
			getDescription(plist[currentId].split("~~~")[1]);
					  			...
					  			<Table>
					  				<tr>
								  		<td align="left" valign="top" style="background-color: white;width:20px;">
								  			<img src="<%=photoOwnerInfo.getImagePath()%>" style="width: 70px;" height="50px"/>
								  		</td>
					  												...
					  												<td style="background-color: green; position: absolute;top:510px;"  > 
					  													<div class="newComment" style="vertical-align: top;">
																					  ...
																					  <table border="0" cellpadding="0" cellspacing="0" style="vertical-align: top;">
																					  	<tr valign="top">
																					  		<td valign="top">
																					  			<img src="<%=(emp.getImagePath()==null?"images\\person_icon.jpg":emp.getImagePath()) %>" width="35px"/>
																					  		</td>
